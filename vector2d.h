...
vector2d *vector2d_new(int column_length, int column_init_capacity, float column_capacity_extend_times) {
    vector2d *columns = vector_new(sizeof(vector), column_length, column_init_capacity, column_capacity_extend_times);
    assert(columns != NULL);
    return columns;
}

bool vector2d_row_push(vector2d *vec2d, int sizeof_element, int length, int init_capacity, float capacity_extend_times) {
...
bool vector2d_row_element_push(vector2d *vec2d, int column_index, void *in_row_value) {
...
bool vector2d_row_element_pop(vector2d *vec2d, int column_index, void *out_row_value) {
    assert(column_index >= 0 && column_index < vec2d->element_length);
    vector *row = VECTOR2D_GET_ROW(vec2d, column_index);
    return VECTOR_POP(row, out_row_value);
}

bool vector2d_set(vector2d *vec2d, int row_index, int column_index, void *in_value) {
    assert(column_index >= 0 && column_index < vec2d->element_length);
    vector *row = VECTOR2D_GET_ROW(vec2d, column_index);
    return VECTOR_SET(row, row_index, in_value);
}

bool vector2d_get(vector2d *vec2d, int row_index, int column_index, void *out_value) {
    assert(column_index >= 0 && column_index < vec2d->element_length);
    vector *row = VECTOR2D_GET_ROW(vec2d, column_index);
